...
class PairDataset(Dataset):
    ...
    参数：
      ...
      max_samples: 限制加载的样本数（None 表示全量）
    """
...
def build_pair_loaders(data_dir, tokenizer, max_length=64, batch_size=32, max_samples=None):
    ...
    max_samples: 限制训练集样本数（None 表示全量）
    """
    data_dir = Path(data_dir)
    train_ds = PairDataset(data_dir / "train.jsonl",      tokenizer, max_length, max_samples=max_samples)
...
def build_triplet_loader(data_dir, tokenizer, max_length=64, batch_size=32, max_samples=None):
    ...
    data_dir = Path(data_dir)
    train_ds = TripletDataset(data_dir / "train.jsonl", tokenizer, max_length, max_samples=max_samples)
    val_ds   = PairDataset(data_dir / "validation.jsonl", tokenizer, max_length)
...
def build_crossencoder_loaders(data_dir, tokenizer, max_length=128, batch_size=32, max_samples=None):
    ...
    data_dir = Path(data_dir)
    train_ds = CrossEncoderDataset(data_dir / "train.jsonl",      tokenizer, max_length, max_samples=max_samples)
