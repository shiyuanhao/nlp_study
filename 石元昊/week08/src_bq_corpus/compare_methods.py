...
METHODS = [
    {
        ...
        "label":     "BiEncoder\n(CosineEmbeddingLoss)",
        "ckpt":      f"{DATASET_NAME}_biencoder_cosine_best.pt",
        "type":      "biencoder",
        "color":     "#2196F3",
    },
    {
        ...
        "label":     "BiEncoder\n(TripletLoss)",
        "ckpt":      f"{DATASET_NAME}_biencoder_triplet_best.pt",
        "type":      "biencoder",
        "color":     "#4CAF50",
    },
    {
        "key":       "crossencoder",
        "label":     "CrossEncoder\n(CrossEntropyLoss)",
        "ckpt":      f"{DATASET_NAME}_crossencoder_best.pt",
        "type":      "crossencoder",
        "color":     "#FF9800",
    },
]
...
def plot_comparison_bar(results, save_path):
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title(f"Method Comparison on {DATASET_NAME.upper()} Validation (4-layer BERT, 3 epochs)")
    ax.legend()
...
def plot_sim_distributions(biencoder_results, save_path):
    ...
    fig.suptitle(f"{DATASET_NAME.upper()} BiEncoder Similarity Distribution", y=1.01)
    fig.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="三种文本匹配方法效果对比（BQ Corpus）")
...
def main():
    ...
    print(f"设备: {device}  数据集: {DATASET_NAME}  评估集: {args.split}")
    ...
    print(f"\n{'='*65}")
    print(f"[{DATASET_NAME.upper()}] 方法对比")
    ...
    log = [{k: _to_py(v) for k, v in m.items() if k not in SKIP_KEYS}
           for m in all_results]
    log_path = LOG_DIR / f"{DATASET_NAME}_method_comparison.json"
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    with open(log_path, "w", encoding="utf-8") as f:
        json.dump(log, f, ensure_ascii=False, indent=2)
    print(f"\n对比日志 → {log_path}")

    plot_comparison_bar(all_results, FIG_DIR / f"{DATASET_NAME}_method_comparison_bar.png")

    bi_with_sim = [m for m in all_results if m["type"] == "biencoder" and "similarities" in m]
    if bi_with_sim:
        plot_sim_distributions(bi_with_sim, FIG_DIR / f"{DATASET_NAME}_biencoder_sim_distributions.png")
